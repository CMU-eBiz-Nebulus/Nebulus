...
- (IBAction)playButtonClicked:(id)sender {
        ...
        
    {
        ...
        [audioTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, audioAsset.duration)
                            ofTrack:[[audioAsset tracksWithMediaType:AVMediaTypeAudio]objectAtIndex:0]
                             atTime:CMTimeMakeWithSeconds([(NSNumber*)[l objectAtIndex:i] floatValue]/15, 600)
                              error:&error];
    ...
    [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 100) queue:nil
                                     usingBlock:^(CMTime time){
                                         ...
                                         self.timeLine.frame=CGRectMake(_player.currentTime.value/_player.currentTime.timescale*15, 0, 1, 300);
                                         [self.timeLine setNeedsDisplay];
                                     }];
